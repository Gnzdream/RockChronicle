...
//		Gdx.app.log("CharacterBuilder", String.format("扫描角色共 %d 个", defs.size));
	 ...
	 * 角色名 - 角色预定义的初始化数据封装类
	 */
	private final ObjectMap<String, Class<? extends Foe>> defs = new ObjectMap<>();
...
//	}
	 ...
	 *   当 name 对应的人物数据不存在时
	 */
	public Foe create(String name, JsonValue customData) {
		Class<? extends Foe> def = defs.get(name);
		...
		try {
			Constructor<? extends Foe> constractors = def.getConstructor(JsonValue.class);
			Foe entry = constractors.newInstance(customData);

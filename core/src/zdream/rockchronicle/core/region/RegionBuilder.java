...
import static zdream.rockchronicle.core.region.Terrains.terrainCode;
...
public class RegionBuilder {
	 ...
	 */
	public ObjectMap<String, SoftReference<Region>> regionBuf = new ObjectMap<>();
	...
	public Region build(String name) {
		// 看看缓存中是否有
		SoftReference<Region> ref = regionBuf.get(name);
		if (ref != null) {
			Region r = ref.get();
			if (r != null) {
				return r;
		...
		regionBuf.put(name, new SoftReference<Region>(r));

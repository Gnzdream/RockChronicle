...
public class MegamanHealthMiscPainter implements IPainter {
	...
	public void draw(SpriteBatch batch, OrthographicCamera camera) {
		...
		int xstart = 16;
		int ystart = app.heightInPixel - entry.height - 24;
		...
		// 黑底
		entry = textures.entrys.get("black");
		batch.draw(black, xstart + 7, ystart + 23,
				entry.width, entry.height * 28);
		
		// 血条
		entry = textures.entrys.get("health");
		health.setRegion(entry.region, 0, entry.height - hp * 3, entry.width, hp * 3);
		batch.draw(health, xstart + 7, ystart + 23, entry.width, hp * 3);
//		batch.d
		if (hpPlus) {
			entry = textures.entrys.get("health_red");
			batch.draw(healthRed, xstart + 7, ystart + 23 + hp * 3,
					entry.width, entry.height);
		}

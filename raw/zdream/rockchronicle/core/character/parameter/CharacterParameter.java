...
public class CharacterParameter implements IValueCreator<JsonValue> {
	...
	public CharacterParameter setBoxAnchorP(int px, int py) {
		JsonValue current = value;
		current = createIfNotExist(current, "box");
		current = createIfNotExist(current, "anchor");
		
		current.addChild("px", new JsonValue(px));
		current.addChild("py", new JsonValue(py));
		return this;
	}
	
	public CharacterParameter setMotionVelocityP(int px, int py) {
		JsonValue current = value;

...
public class ClimbModule extends AbstractModule implements IMovable {
	...
	public void init(FileHandle file, JsonValue value) {
		...
		climbVelocity = (int) (fv * TIME_STEP * P_PER_BLOCK);
	...
	public void determine(LevelWorld world, int index, boolean hasNext) {
		...
		if (climbing == 0) {
			...
			byte terrain = world.getTerrain(centerX, centerY);
			if (Terrains.isLadder(terrain)) {
				// 这个时候如果按了上或者下, 就应该到梯子上了, 除了一种情况: 你在落地时按下
				if (box.gravityDown && occ.ybottomTightly) {
					if (world.getTerrain(centerX, occ.ybottom - 1) == TERRAIN_SOLID
							&& upOrDown == 2) {
					} else {
						this.adhere = true;
					}
				} else if (!box.gravityDown && occ.ytopTightly) {
					if (world.getTerrain(centerX, occ.ytop + 1) == TERRAIN_SOLID
							&& upOrDown == 1) {
					} else {
						this.adhere = true;
					}
				...
				if (box.gravityDown && occ.ybottomTightly) {
					if (Terrains.isLadder(world.getTerrain(centerX, occ.ybottom - 1)) && upOrDown == 2) {
						// 可以向下降到梯子上
						lower = true;
						lowerX = (int) ((centerX + 0.5f) * P_PER_BLOCK);
						lowerY = occ.ybottom;
					}
				} else if (!box.gravityDown && occ.ytopTightly) {
					if (Terrains.isLadder(world.getTerrain(centerX, occ.ytop + 1)) && upOrDown == 1) {
						// 人是倒着的, 可以向上“降”到梯子上
						lower = true;
						lowerX = (int) ((centerX + 0.5f) * P_PER_BLOCK);
						lowerY = occ.ytop + 1;
		...
		if (climbing <= 7) {
			byte terrain = world.getTerrain(anchorX, anchorY);
			...
			if (Terrains.isLadder(terrain)) {
				...
				terrain = world.getTerrain(centerX, centerY); // 检查中点
				if (!Terrains.isLadder(terrain)) {
					// 梯子消失 ?
					climbing = 0; // 变成站立
					parent.getBoxModule().setNextPattern("normal");
		...
		if (world.currentRoom.containInRoomForBlock(pCenterX, pCenterY) == -1) {
			climbing = 0;
			parent.getBoxModule().setNextPattern("normal");
			setClimbState(); return;
		}
	...
	public void action(LevelWorld world, Box box, CharacterEntry entry) {
		...
		if (climbing == 0) {
			if (adhere) {
				...
				if (box.gravityDown && occ.ybottomTightly &&
						Terrains.isLadder(world.getTerrainForP(lowerX, lowerY - 1))) {
				...
				if (!box.gravityDown && occ.ybottomTightly &&
						Terrains.isLadder(world.getTerrainForP(lowerX, lowerY))) {
					parent.getBoxModule().setNextPattern("climb_top_1");
					box.setAnchorX(lowerX);
		...
		if (climbing >= 2) {
			...
			if (climbing > 13) {
				...
				// 回到一般状态
				parent.getBoxModule().setNextPattern("normal");
				box.setAnchorY(Box.pCeil(box.anchorY) - (int) (P_PER_BLOCK * 0.55f));
			} else {
				if (last == 7 && climbing == 8) {
					parent.getBoxModule().setNextPattern("climb_top_1");
					box.setAnchorY(Box.pCeil(box.anchorY));
				} else if (last == 8 && climbing == 7) {
					parent.getBoxModule().setNextPattern("climb_top_0");
					box.setAnchorY(Box.pCeil(box.anchorY) - (int) (P_PER_BLOCK * 0.25f));
				}
			}
		...
		// 平移到梯子上;
		box.setAnchorX(px + P_PER_BLOCK / 2);
		
		// 将根据角色与梯子顶端的距离来设置爬梯子状态;
		
		// 下面需要粗略计算离梯子顶端的距离 (这里的 y 以角色锚点为准)
		int iy = Box.pCeil(box.anchorY);
		boolean yTightly = box.anchorY == iy;
		int distance = (iy - box.anchorY) +
				(yTightly ?
				(Terrains.isLadder(world.getTerrainForP(px, iy + P_PER_BLOCK)) ? 1 : 0) :
				(Terrains.isLadder(world.getTerrainForP(px, iy)) ? 1 : 0));

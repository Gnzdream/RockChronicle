...
import static zdream.rockchronicle.platform.region.Gate.DIRECTION_BOTTOM;
import static zdream.rockchronicle.platform.region.Gate.DIRECTION_LEFT;
import static zdream.rockchronicle.platform.region.Gate.DIRECTION_RIGHT;
import static zdream.rockchronicle.platform.region.Gate.DIRECTION_TOP;
			...
			{ // 阶段二: 定 x 轴
				...
				if (g.direction == DIRECTION_RIGHT) {
					...
					for (int i = 0; i < param.entries.size; i++) {
						...
						Box box = entry.getBoxModule().getBox();
						float xleft = Box.p2block(box.posX);
						float anchorX = Box.p2block(box.anchorX);
						float anchorY = Box.p2block(box.anchorY);
						
						// 向右移屏后, 角色左边需要等于 1
						param.phase2EntryWidth[i] = destRoom.offsetx - g.offsetXOfRegion - srcRoom.offsetx + 1 - xleft;
						param.entriesPos[i] = new Vector2(anchorX + param.phase2EntryWidth[i], anchorY);
					}
				} else {
					...
					for (int i = 0; i < param.entries.size; i++) {
						CharacterEntry entry = param.entries.get(i);
						Box box = entry.getBoxModule().getBox();
						float xright = Box.p2block(box.posX + box.posWidth); // 相对于 srcRoom
						float anchorX = Box.p2block(box.anchorX);
						float anchorY = Box.p2block(box.anchorY);
			...
			{ // 阶段二: 定 y 轴
				...
				if (g.direction == DIRECTION_BOTTOM) { // 向下
					...
					for (int i = 0; i < param.entries.size; i++) {
						CharacterEntry entry = param.entries.get(i);
						Box box = entry.getBoxModule().getBox();
						float ybottom = Box.p2block(box.posY);
						float boxHeight = Box.p2block(box.posHeight);
						float anchorX = Box.p2block(box.anchorX);
						float anchorY = Box.p2block(box.anchorY);
						
						// 向下移屏后, 角色上边需要等于目标房间高 - 0.1
						param.phase2EntryWidth[i] = (srcRoom.offsety + ybottom)
								- (destRoom.offsety - g.offsetYOfRegion + destRoom.height - 0.1f - boxHeight);
						param.entriesPos[i] = new Vector2(anchorX, anchorY - param.phase2EntryWidth[i]);
					}
				} else {
					...
					for (int i = 0; i < param.entries.size; i++) {
						CharacterEntry entry = param.entries.get(i);
						Box box = entry.getBoxModule().getBox();
						float ybottom = Box.p2block(box.posY); // 相对于 srcRoom
						float anchorX = Box.p2block(box.anchorX);
						float anchorY = Box.p2block(box.anchorY);
		...
		if (param.phase == 2) {
			...
			for (int i = 0; i < param.entries.size; i++) {
				...
				if (bx) {
					float blockX;
					if (delta >= 0) {
						blockX = (param.entriesPos[i].x - remain * param.phase2EntryWidth[i]);
					} else {
						blockX = (param.entriesPos[i].x + remain * param.phase2EntryWidth[i]);
					}
					box.setAnchorX(Box.block2P(blockX));
				} else {
					float blockY;
					if (delta >= 0) {
						blockY = (param.entriesPos[i].y - remain * param.phase2EntryWidth[i]);
					} else {
						blockY = (param.entriesPos[i].y + remain * param.phase2EntryWidth[i]);
					}
					box.setAnchorY(Box.block2P(blockY));
				}
			}
		}
